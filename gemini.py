...
from utils.classes_funcions import compare_classes, replace_method, extract_method_signature, extract_options, \
    extract_method_name_from_signature
...
API_KEY = "AIzaSyAw9c5TYAtJdnRhhjS-tkglWAtsQEL8Goc"
...
def generate_mutants(fixed_code, vulnerable_code, mutated_path, class_path, vul_id, container):
    prompt = (f"Create a mutant for the following method by applying one of the mutation operators used for mutation "
              f"testing in the code line marked with //MUTATE. "
              f"The task is to improve code testing by checking whether changes made to fixed code introduce or "
              f"detect vulnerabilities. Not produce a mutation "
              f"that results in the same code as fixed code.\n Fixed code: {fixed_code}\n"
              f"Vulnerable code: {vulnerable_code}\n Please consider changes such as using different operators, "
              f"changing constants, referring to different variables, object properties, functions, or methods. "
              f"Provide three answers as fenced code blocks containing all the code given in input"
              f" including the mutated lines, using the following template: "
              f"Option 1: The lines can be replaced with: <code fragment> "
              f"This would result in different behavior because <brief explanation>. "
              f"Option 2: The lines can be replaced with: <code fragment> "
              f"This would result in different behavior because <brief explanation>. "
              f"Option 3: The lines can be replaced with: <code fragment> "
              f"This would result in different behavior because <brief explanation>.")
    ...
    print("\nClass to mutate: ", source_class_path)
    ...
    # Creazione delle tre versioni della classe Java per ogni metodo mutato
    for i, option in enumerate(options, 1):
        ...
        with open(class_path, 'r', encoding='utf-8') as class_content:
            modified_class_content = replace_method(class_content.read(), option, signature)
...
def get_compilation_results(path_dir):
    bool_compiled = None
    path_compile_result_text_file = path_dir + "/compile_result.txt"
...
def get_test_results(path_dir):
    test_passed = None
    path_testing_results_json_file = path_dir + "/testing_results.json"
...
def execute_tests_and_get_results(mutant_path, vul_id, container):
    ...
    bool_compiled = get_compilation_results(str(mutant_path.parent))
    ...
    if bool_compiled is None:
        ...
        bool_test_passed = get_test_results(path_dir + "/" + vul_id + "/VUL4J")
...
def get_cve_id_and_failed_test_details(path_dir, vul_id):
    dict_failed_tests = {}
    path_testing_results_json_file = path_dir + "/" + vul_id + "/VUL4J/test_result/testing_results.json"
    cve_id = ""
...
def get_mutant_vulnerability_comparison_scores(vul_id, changed_file_dir, mutant_id, dict_failed_tests_by_vuln):
    did_mutant_compile = False
    did_tests_fail = False
    imitates_vuln = False
    ochiai = 0
    failed_tests_intersection = "\"\""
    path_mutants_dir_for_vulnerability = "vul4j/code_dir/" + vul_id + "/VUL4J/mutated_code/" + changed_file_dir
    print("path_mutants_dir_for_vulnerability: ", path_mutants_dir_for_vulnerability)
    ...
    if bool_compiled is True:
        ...
        if bool_test_passed is False:
            did_tests_fail = True
            cve_id, dict_failed_tests_by_mutant = get_cve_id_and_failed_test_details(path_mutants_dir_for_vulnerability,
                                                                                     mutant_id)
...
def analyze_mutants_simulations_and_get_analysis():
    ...
    for vul_id in os.listdir('vul4j/code_dir'):
        ...
        mutants_path = "vul4j/code_dir/" + vul_id + "/VUL4J/mutated_code/"
        if not os.path.exists(mutants_path):
            print(mutants_path, "not exist")
            continue
        for changed_file_dir in os.listdir(mutants_path):
            cve_id, dict_failed_tests_by_vuln = get_cve_id_and_failed_test_details('vul4j/code_dir', vul_id)
            if len(dict_failed_tests_by_vuln) == 0:
                print("NO FAILED TESTS")
                continue

            for mutant_id in os.listdir(mutants_path + "/" + changed_file_dir):
                if os.path.isdir(mutants_path + "/" + changed_file_dir + "/" + mutant_id) is False:
                    continue
                ...
                did_mutant_compile, did_tests_fail, imitates_vuln, ochiai, failed_tests_intersection, failed_tests, failures, failed_tests_vuln, failures_vuln, vuln_coupling_result = \
                    get_mutant_vulnerability_comparison_scores(vul_id, changed_file_dir, mutant_id,
...
def plot(df_analysis_csv):
    if os.path.exists("vul4j/analysis/plot_ochiai.pdf") == False:
...
def plot_coupling(df_analysis_csv):
    if os.path.exists("vul4j/analysis/plot_coupling.pdf") == False:
...
def plot_venn_vulnerability (data):
    if os.path.exists("vul4j/analysis/plot_venn.pdf") == False:
...
def main():
    ...
    for vul_id in os.listdir(base_directory):
        print("Vulnerabilities to be analysed: ", vul_id, "\n")
        ...
        for file in os.listdir(vuln_directory):
            if not file.endswith('.java'):
                continue
            file_to_mutate_path = mutated_path.joinpath(file.replace(".java", ""))
            if not file_to_mutate_path.exists():
                file_to_mutate_path.mkdir(parents=True, exist_ok=True)
            ...
            if fixed_methods and vuln_methods:
                for fixed, vuln in zip(fixed_methods, vuln_methods):
                    ...
                    generate_mutants(fixed, vuln, file_to_mutate_path, fixed_directory.joinpath(file), vul_id, container)
            else:
